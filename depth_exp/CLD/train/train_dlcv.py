"""
Training script for CLD using DLCV dataset.
This script trains only the MultiLayer-Adapter (MLCA) using DLCVCLDDataset.
"""
...
# Add project root to path for importing DLCV dataset
...
def train(config_path):
    ...
    # Only inject LoRA into MultiLayer-Adapter (MLCA training only)
    set_lora_into_transformer(pipeline.multiLayerAdapter, lora_rank, lora_alpha, lora_dropout)
    
    # Freeze transformer completely (only train MLCA)
    pipeline.transformer.requires_grad_(False)
    pipeline.transformer.eval()
    
    # Only train MultiLayer-Adapter
    ...
    print(f"[INFO] MLCA-only training. MultiLayer-Adapter Trainable params: {n_trainable_adapter/1e6:.2f}M", flush=True)

    print("[INFO] Using Prodigy optimizer for MLCA only.", flush=True)
    # Only create optimizer for adapter (no transformer optimizer)
    ...
    # Load DLCV dataset
    print("[INFO] Loading DLCV dataset...", flush=True)
    ...
    out_dir = config.get("output_dir", "./mlca_lora_out")
    ...
    if "resume_from" in config and config["resume_from"] is not None:
        ...
        # For MLCA-only training, we only load adapter checkpoint
        # Create dummy optimizer/scheduler for transformer to satisfy load_checkpoint signature
        # (they won't be used since transformer is frozen)
        dummy_optimizer = Prodigy([], lr=1.0)
        dummy_scheduler = LambdaLR(dummy_optimizer, lr_lambda=lambda step: 1.0)
        start_step = load_checkpoint(
            pipeline.transformer, 
            pipeline.multiLayerAdapter, 
            dummy_optimizer,  # Dummy, won't be used
            optimizer_adapter, 
            dummy_scheduler,  # Dummy, won't be used
            scheduler_adapter, 
            ckpt_dir, 
            device
        )
    pbar = tqdm(total=max_steps, desc="train_mlca", initial=start_step)
    step = start_step
    ...
    while step < max_steps:
        for batch in loader:
            ...
            # Set transformer to eval mode (frozen, but still compute gradients for adapter)
            pipeline.transformer.eval()
            pipeline.multiLayerAdapter.train()

            # Forward pass: adapter needs gradients, transformer is frozen but still computes gradients
            # (gradients flow through transformer but won't update its parameters)
            (
            ...
            if (step + 1) % accum_steps == 0:
                torch.nn.utils.clip_grad_norm_(params_adapter, max_norm=float(config.get("grad_clip", 1.0)))
            ...
            if step % save_every == 0 or step == max_steps:
                # Save checkpoint (only adapter will have trainable params, transformer is frozen)
                # Create dummy optimizer/scheduler for transformer to satisfy save_checkpoint signature
                dummy_optimizer = Prodigy([], lr=1.0)
                dummy_scheduler = LambdaLR(dummy_optimizer, lr_lambda=lambda step: 1.0)
                save_checkpoint(
                    pipeline.transformer,  # Pass transformer for compatibility
                    pipeline.multiLayerAdapter, 
                    dummy_optimizer,  # Dummy, transformer has no trainable params
                    optimizer_adapter, 
                    dummy_scheduler,  # Dummy, transformer has no trainable params
                    scheduler_adapter, 
                    step, 
                    out_dir
                )
    ...
    pbar.close()
    print("[DONE] MLCA training finished.")
